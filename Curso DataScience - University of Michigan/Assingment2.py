...
def answer_three():
    cdf = df[(df['Gold'] > 0) & (df['Gold.1'] > 0)]
    return ((cdf['Gold'] - cdf['Gold.1']) / cdf['Gold.2']).argmax()
...
def answer_six():
    ...
    states_df['State'] = cdf['STNAME'].unique()
    states_df.set_index('State', inplace=True)
    states_df['PopSum'] = 0
    for i in states_df.index:
        countcity = cdf[cdf['STNAME'] == i].sort_values(['CENSUS2010POP'], ascending=False)
        if countcity['COUNTY'].count() == 1:
            states_df['PopSum'][states_df.index == i] = countcity['CENSUS2010POP']
        
        states_df['PopSum'][states_df.index == i] = sum(countcity.iloc[:3]['CENSUS2010POP'])
        
    states_df = states_df.sort_values(['PopSum'], ascending=False).head(3)
    
    return list(states_df[:3].index)
